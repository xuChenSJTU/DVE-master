...
dataset_file = 'wiki_small'  # 'epinions_small', 'Slashdot_small', 'wiki_small'
ratio = 1.0
latent_d = 64
n_sample_noise = 20
use_feature = False
...
parser.add_argument('--n_embedding', dest='n_embedding', type=int, default=128, help='dimension of embedding vector')
...
if __name__ == '__main__':
    ...
    args.n_embedding = latent_d  # default latent_d = 128

    node2idx = pickle.load(open(os.path.join(os.getcwd(), 'output', '{}_node2idx.pkl'.format(dataset_file)), 'rb'))
    # construct node2idx df
    node2idx_df = pd.Series(list(node2idx.values()), index=[str(int(float(ele.decode()))) for ele in list(node2idx.keys())])
    ...
    for i in range(n_fold)[:1]:
        ...
        with tf.Session(config=config) as sess:
            ...
            precision5_list, \
            precision10_list, \
            precision20_list = model.train(args, feed_data, train_links_df, test_links_df, topN_list)
