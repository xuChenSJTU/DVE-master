...
from DVE_model import VGE_EDGE
import File_Reader
import pickle
import utils
...
dataset_file = 'epinions_small'  # 'epinions_small', 'Slashdot_small', 'wiki_small'
ratio = 1.0
latent_d = 64
n_sample_noise = 5
use_feature = False
...
parser.add_argument('--keep_prob', dest='keep_prob', type=float, default=0.2, help='prob of keeping activitation nodes')
...
if __name__ == '__main__':
    print('################### dataset: {}, ratio: {}, noise: {}, lr: {}, latent_d: {} #############'.format(dataset_file, ratio, n_sample_noise, args.learning_rate, latent_d))
    args.n_hidden_list = [2*latent_d, latent_d]  # default latent_d = 64
    print('number of layers: {}'.format(len(args.n_hidden_list)))
    ...
    # print('n noise: {}, choose ratio: {}'.format(n_sample_noise, choose_ratio))
    ...
    for i in range(n_fold)[:1]:
        ...
        with tf.Session(config=config) as sess:
            model = VGE_EDGE(sess, n_nodes, args)
            auc_sign_list, \
            f1_sign_list, \
            recall5_list, \
            recall10_list, \
            ...
            precision10_list, \
            precision20_list,\
            epoch_loss_list = model.train(args, train_sign_pos_sp, train_sign_neg_sp, feed_data, train_links_df,
                                            test_links_df, topN_list)

        # save training loss and test res for different dropout params
        pickle.dump(epoch_loss_list, open(os.path.join(os.getcwd(), 'result', 'train_loss_list_dropout_{}_{}.pkl'.
                                                     format(args.keep_prob, dataset_file)), 'wb'))
        pickle.dump(auc_sign_list, open(os.path.join(os.getcwd(), 'result', 'test_auc_list_dropout_{}_{}.pkl'.
                                                     format(args.keep_prob, dataset_file)), 'wb'))
        pickle.dump(recall20_list, open(os.path.join(os.getcwd(), 'result', 'test_recall50_list_dropout_{}_{}.pkl'.
                                      format(args.keep_prob, dataset_file)), 'wb'))
    ...
    print('################### dataset: {}, ratio: {}, noise: {}, lr: {}#############'.format(dataset_file, ratio,
                                                                                              n_sample_noise,
                                                                                              args.learning_rate))
    print('number of layers: {}'.format(len(args.n_hidden_list)))
    print(
        '################### dataset: {}, ratio: {}, noise: {}, lr: {}, latent_d: {} #############'.format(dataset_file,
                                                                                                           ratio,
                                                                                                           n_sample_noise,
                                                                                                           args.learning_rate,
                                                                                                           latent_d))