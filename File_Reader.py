...
def normalize_sign_adjacency(adjacency):
    coo_adjacency = sp.coo_matrix(adjacency)
    adjacency_ = coo_adjacency + sp.eye(coo_adjacency.shape[0])
    degree = np.array(np.abs(adjacency_).sum(1))
    d_inv = sp.diags(np.power(degree, -0.5).flatten())
    normalized = adjacency_.dot(d_inv).transpose().dot(d_inv)
...
def dw_normalize_adjacency(adjacency):
    coo_adjacency = sp.coo_matrix(adjacency)
    vol_G = np.sum(coo_adjacency)
    adjacency_ = coo_adjacency
    degree = np.array(adjacency_.sum(1))
    d_inv_flatten = np.power(degree, -0.5).flatten()
    d_inv_flatten[np.isinf(d_inv_flatten)] = 0.0
    d_inv = sp.diags(d_inv_flatten)
    normalized = adjacency_.dot(d_inv).transpose().dot(d_inv)

    return dense_to_sparse(normalized), dense_to_sparse(d_inv), vol_G * 1.0
...
def train_test_split_link(adjacency, train_ratio):
    ...
    num_test = int(np.floor(test_ratio * edges.shape[0]))
    ...
    train_edges = np.delete(edges, np.hstack([idx_test]), axis=0)
    ...
    return train_adjacency, test_edges_pos, test_edges_neg


def train_test_split_class(adjacency, train_ratio):
    n_nodes = adjacency.shape[0]
    coo_adjacency = sp.coo_matrix(adjacency)
    coo_adjacency_upper = sp.triu(coo_adjacency, k=1)
    sp_adjacency = dense_to_sparse(coo_adjacency_upper)
